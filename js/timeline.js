  ...
  firebase.database().ref(lista.title).on('value', function (snapshot) {
    ...
    snapshot.forEach(function (item) {
      ...
      + '<div class="card-content white-text">'
        ...
        + '<h5>' + item.val().raca + '</h5>'
        + '<p>' +'R$'+item.val().preco + '</p>'
        + '<p>' +'Endereço: '+item.val().logradouro + ' '+ item.val().numero + ' ' + item.val().bairro+'</p>'
        + '<p>' +'CEP: '+item.val().cep + '</p>'
        + '<p>' + 'Descrição: '+item.val().infoComplementar + '</p>'


        + '<a class="btn-floating halfway-fab waves-effect waves-light cyan accent-4"><i class="material-icons">shop</i></a>'
